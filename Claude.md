...
### Phase 6A — Multi-Chair Routing — In Progress
...
- Still to do: Step 2 (`chair_router.py` — `ChairRoutingStrategy` ABC + `IntentMappingStrategy` + `CHAIR_ROUTING_STRATEGY` flag, wired where lane==human_review) · Step 3 (chair-reroute path + audit shape confirmation + toy dataset + integration tests) · then 6B (frontend: chair display + reroute-to-chair UI).
- Status: 111/111 backend tests passing (test_eval_harness ground-truth count 58→67 updated).

---

## Current Status — Phases 0–4 COMPLETE · Phase 5A–5I COMPLETE · Phase 6A IN PROGRESS · 111/111 tests passing
...
| Phase 6A | In Progress | Multi-chair routing. Step 1 ✅ DB migration (Chair table + Email.assigned_chair_id FK, 5 chairs seeded). Step 1.5 ✅ classifier taxonomy 8→**11 intents** (+sponsorship/publicity/media_inquiry) → Publicity/Sponsorship Chair now auto-routable; eval set 58→67, zero regression on original 8, new 3 at F1 1.000. Next: Step 2 chair_router.py · Step 3 reroute+toy+integration tests — 111/111 |
