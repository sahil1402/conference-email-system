...
- **Zendesk integration is now COMPLETE**: Pieces 1–5 built, mock-tested, AND proven end-to-end with real data; overlap guard hardened. Remaining before production is operational only: apply migration `f7a1b2c3d4e5` to the real DB, and decide on enabling background polling / `ALLOW_AUTO_SEND` (both stay off by default).
...
### 2026-07-20 — Intent taxonomy adoption + FAQ-lane rework (draft-quality gate) — Complete on `feature/intent-taxonomy-adoption` (NOT merged; awaiting merge decision)
...
## Current Status — Phases 0–6C COMPLETE · Real-Corpus + Phase 7 COMPLETE · Zendesk Pieces 1–5 COMPLETE + proven live end-to-end (all on main) · main 265 passed / 6 skipped (`-m "not ml"`) · `feature/production-hosting-v2` 192/192 · frontend build clean
...
| Zendesk overlap guard | Complete · migration not applied to demo | Single-flight lock on `zendesk_sync_state` (`is_running`+`running_since`, `try_acquire_lock` w/ `FOR UPDATE`) · 2nd trigger → 409 "already in progress" · 900s staleness takeover for crashed runs · migration `f7a1b2c3d4e5` created but **NOT applied to demo Postgres** (gate; demo at `c361c1d3ad79`) · local 265/265 |
...
| Intent taxonomy + FAQ-lane rework | Complete · on `feature/intent-taxonomy-adoption` (not merged) | 14-intent taxonomy single-sourced in `taxonomy.py` (coverage map; intent prior gated OFF per E010) **+** FAQ lane decided by DRAFT QUALITY not intent: drafter self-rated `answer_confidence`, route-after-draft, `FAQ_ELIGIBLE_INTENTS` retired, `SENSITIVE_INTENTS=[]` seam, strategy-independent self-sufficiency floor, chair reseed mig `b2c3d4e5f6a7` (single head), run_eval end-to-end (E011). Whole-branch review READY TO MERGE · 325 passed/7 skipped (incl ml) |
...
- **Zendesk integration COMPLETE — remaining items are operational only** — Pieces 1–5 built, mock-tested, and **proven end-to-end with real data** (live internal-note write-back to ticket 21567 verified 2026-07-19); overlap guard hardened. Before production: (1) apply migration `f7a1b2c3d4e5` (overlap-lock columns) to the real DB — created but deliberately NOT applied to the demo Postgres (gate; demo at `c361c1d3ad79`); (2) decide whether to enable background polling (`ZENDESK_POLLING_ENABLED`, default `False`) and/or `ALLOW_AUTO_SEND` for public auto-replies (default `False`) — both remain off by design. Nothing else outstanding.
