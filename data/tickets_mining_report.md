...
> **The figures in this section and the table below are the state as of the manual decisions, and have since been superseded.** Two later refinement passes changed three intents. **Current corpus totals: 113 clusters / 2,908 clustered / 747 noise / 79.6% coverage.** Per intent: `reviewer_assignment` **15 clusters / 168 noise / 71.9%**, `reviewer_workload_role` **9 / 44 / 89.7%**, `author_profile_compliance` **10 / 0 / 100%**. See the two addenda at the end of this section; **Addendum 2 carries the current numbers**.
...
> **Partly resolved later** — the `reviewer_assignment` case below WAS decomposed by reducing dimensionality first (UMAP); see the E012 addendum at the end of this section. The limitation still holds for outcome polarity.
...
### Addendum — E012 targeted UMAP decomposition (applied later)
A follow-up experiment applied **UMAP as a targeted decomposition tool — not a pipeline-wide replacement** — to resolve `reviewer_assignment`'s one documented residual-mixture case, the same case the *Known limitation* above records as undecomposable. Full reasoning: **`docs/exp_tracking/E012_stage3_umap_dbcv.md`**.

- **What it fixed.** Re-clustering that residual in isolation under `UMAP(n_components=10, n_neighbors=15)` + `HDBSCAN(mcs=20)` split the 136-ticket / 61%-emergency blob that raw 384-dim space could not break at any `mcs` into **67 @ 10% / 65 @ 92% / 20 @ 85% / 20 @ 80%** emergency. Verified against an emergency-purity metric **independent of both UMAP and the selection metric**, then reviewed by reading real `steps_taken`.
- **Applied**: group 1 (n=65) **folded** into the emergency-invite cluster (73 → **138**) — procedurally identical step-for-step, not merged on centroid similarity alone; **3 new clusters** added (grant-reassignment n=67, reviewer's-own-status n=20, who-may-invite n=19); the n=180 residual removed; **8** members returned to noise. One hand correction: ticket **17955** moved to the decline-late-requests cluster (n=22 → 23), its steps being that procedure rather than the new who-may-invite one.
- **Updated corpus-wide totals**: **113 clusters** (6 primary + 9 recovered for this intent), **2,938 / 3,655 clustered**, **717 noise**, **80.4% coverage** — from 111 / 2,946 / 709 / 80.6%. `reviewer_assignment` alone: 13 → **15 clusters**, 160 → **168 noise**, 73.2% → **71.9% coverage**. The −8 tickets are the honest cost of not counting a grab-bag as explained.
- **Full UMAP + DBCV-argmax adoption was evaluated and explicitly REJECTED.** Run across all 14 intents, it **disagreed with 3 of the 4 settled human decisions** — it fragmented the merged n=806 outage cluster four ways, merged the `review_submission_help` c7 troubleshooting cluster that was deliberately kept separate, and collapsed `desk_reject_appeal`'s upheld/reversed/triage split into one cluster. Its **selection criterion was also not reproducible**: the DBCV margins it decided on (0.0045 and 0.0083) were **40–50× smaller than the metric's own spread across random seeds** (0.19–0.49), so the winning configurations were seed artefacts — the same class of error as the coverage-blind "stability" criterion recorded above. Scored in the neutral raw embedding space, its partitions were worse than the human-reviewed ones on 11 of 12 intents.
- **Consequence for the *Known limitation* above**: it is narrowed, not lifted. UMAP separated *this* mixture, but it adds no action-type or outcome awareness — `desk_reject_appeal`'s opposite-outcome groups still collapse under it. A distinct action/verb signal is still required for Phase B.

### Addendum 2 — second refinement pass (cohesion scan + singleton audit)
A follow-up scan scored every cluster with n >= 30 on internal cohesion, calibrated against clusters with **known ground truth** (the confirmed `reviewer_assignment` mixture as the lower bound, the human-merged n=806 outage cluster as a clean upper one) rather than an invented threshold. Exactly one cluster fell below the confirmed-mixture boundary, and a parallel audit checked every singleton in the two agglomerative-fallback intents. All four decisions below were made by reading full `steps_taken`, and every membership was asserted as an exact partition before writing.

- **`reviewer_workload_role` c4 (n=91) decomposed.** Its own label already named three actions. Split into **SG2 (20)** *decline a workload reduction, require completion*, **SG1 (19)** *grant it, route to the assignments team*, **SG3 (15)** *acknowledge unavailability, no further action*, plus a **7-ticket** *volunteer inquiry after recruitment closed* core extracted from an otherwise incoherent 25-ticket group. SG1/SG2 are **opposite outcomes of the same request** and are kept apart under the outcome-polarity rule already applied elsewhere in this corpus. 30 members returned to noise (18 from the grab-bag + 12 from the decomposition) — that intent's coverage falls 96.7% → 89.7%, which is the honest figure: the old 96.7% counted a 91-ticket mixture as explained.
- **`author_profile_compliance`: 3 folds, 1 false positive.** Four size<=2 clusters were flagged as possible average-linkage threshold artefacts. Reading them resolved it three ways: `21676` and `21742`/`21855` fold into c0 (13 → 16, label extended to cover the career-stage branch), `21882` folds into **c1** rather than c0 (same chair *action* — adding a submission-form field — not the same advice), and `21874` is **kept separate**: it concerns the declaration form's per-author granularity and presupposes the form is already in use. Shared vocabulary, different question. 13 → 10 clusters, still 100% coverage.
- **`paper_bidding`: reviewed, no change.** c1/c2 kept separate on outcome polarity — one concedes a communication error, the other explains none occurred. Recorded in the intent's entry so it is not mistaken for an unexamined gap.
- **Updated corpus-wide totals**: **113 clusters**, **2,908 / 3,655 clustered**, **747 noise**, **79.6% coverage** (from 113 / 2,938 / 717 / 80.4%). Cluster count is unchanged only by coincidence — `reviewer_workload_role` gained 3 and `author_profile_compliance` lost 3.
- **Methodology finding — `docs/exp_tracking/E013`.** Re-deriving a decomposition in a fresh process moved 9 of 91 tickets with nothing edited: the same texts embedded in a **different-sized batch** differ by ~1e-7 (SentenceTransformer pads per batch), and density-based clustering amplifies that at the noise boundary. A 10-run consensus check shows **cores stable, boundaries jitter ±3**. Cluster sizes from a decomposition should be read as provisional at the edge; reproducing a published number requires reproducing the whole pipeline, batch composition included. **Retrieval is unaffected** — a 1e-7 shift cannot reorder a ranking.
